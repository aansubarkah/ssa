...
def main():
    ...
    def process_video():
        ...
        try:
            ...
            non_silent_ranges = detect_speech(
            ...
            concatenation_success = concatenate_videos(
...
def create_video_clips(input_video, timestamps_file, output_folder):
    ...
    with ThreadPoolExecutor() as executor:
        futures = [executor.submit(process_clip, i, start, end)
                   for i, (start, end) in enumerate(timestamps)]
        for future in as_completed(futures):
            future.result()


def create_video_clips_old_again(input_video, timestamps_file, output_folder):
    os.makedirs(output_folder, exist_ok=True)
    clean_output_folder(output_folder)
    ...
    # Try OpenCV first
    try:
        create_clips_opencv(input_video, timestamps, output_folder)
    except Exception as e:
        print(f"OpenCV failed: {e}. Falling back to moviepy.")
        create_clips_moviepy(input_video, timestamps, output_folder)


def create_clips_opencv(input_video, timestamps, output_folder):
    ...
    def process_clip(i, start, end):
        ...
        while cap.isOpened():
            ret, frame = cap.read()
            if not ret or cap.get(cv2.CAP_PROP_POS_MSEC) > end_sec * 1000:
                break
            out.write(frame)
...
def create_clips_moviepy(input_video, timestamps, output_folder):
    ...
    for i, (start, end) in enumerate(timestamps):
        ...
        try:
            ...
            subclip.write_videofile(
                output_file, codec="libx264", audio_codec="aac", threads=4)
            print(f"Created: {output_file}")
        except Exception as e:
            print(f"Error creating clip {i+1}: {e}")
...
def old_create_video_clips(input_video, timestamps_file, output_folder):
    os.makedirs(output_folder, exist_ok=True)
    clean_output_folder(output_folder)
...
def create_video_clips_old(input_video, timestamps_file, output_folder):
    os.makedirs(output_folder, exist_ok=True)
    clean_output_folder(output_folder)
...
def concatenate_videos(input_folder, output_file):
    ...
    print(
        f"Found {len(video_files)} video files in the input folder: {input_folder}")
