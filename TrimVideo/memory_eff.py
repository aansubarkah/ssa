    ...
    def process_video():
        ...
        try:
            ...
            if not concatenation_success:
                ...
                #progress_bar.update(100, "Error concatenating video clips")
                #return
...
def concatenate_videos(input_folder, output_file):
    ...
    try: 
        ...
        def clip_generator():
            for video_file in video_files:
                clip_path = os.path.join(input_folder, video_file)
                print(f"Loading video file: {clip_path}")
                yield VideoFileClip(clip_path)

        #final_clip = concatenate_videoclips(list(clip_generator()), method="compose")

        #print(f"Writting concatenated video to: {output_file}")
    ...
    #except Exception as e:
        #print(f"Error concatenating video files: {e}")
        #return False
